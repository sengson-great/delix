<ul class="nav pb-12 mb-20" id="pills-tab" role="tablist">
    <li class="nav-item" role="presentation">
        <a class="nav-link ps-0 {{strpos(url()->current(),'merchant/profile') || strpos(url()->current(),'staff/profile') ? 'active':''}}"
    ...
    @if(Sentinel::getUser()->user_type == 'merchant' || (hasPermission('manage_company_information')))
        <li class="nav-item" role="presentation">
            <a class="nav-link ps-0 {{strpos(url()->current(),'merchant/company') || strpos(url()->current(),'staff/company') ? 'active':''}}"
...
{{--                <li><a class="{{strpos(url()->current(),'merchant/notifications')? 'active':''}}" href="{{route('merchant.notifications')}}"><i class="icon ni ni-bell"></i><span>{{__('notifications')}}</span></a></li>--}}
    <li class="nav-item" role="presentation">
        <a class="nav-link ps-0 {{strpos(url()->current(),'merchant/account-activity') || strpos(url()->current(),'staff/account-activity')? 'active':''}}"
    ...
    <li class="nav-item" role="presentation">
        <a class="nav-link ps-0 {{strpos(url()->current(),'merchant/security-settings') || strpos(url()->current(),'staff/security-settings')? 'active':''}}"
    ...
    @if(Sentinel::getUser()->user_type == 'merchant' || (hasPermission('manage_payment_accounts')))
        <li class="nav-item" role="presentation">
            <a class="nav-link ps-0 {{strpos(url()->current(),'merchant/payment/accounts') || strpos(url()->current(),'staff/payment/accounts') ? 'active':''}}"
    ...
    @if(Sentinel::getUser()->user_type == 'merchant' || (hasPermission('delivery_charge')))
        <li class="nav-item" role="presentation">
            <a class="nav-link ps-0 {{strpos(url()->current(),'merchant/charge') || strpos(url()->current(),'staff/charge') ? 'active':''}}"
    ...
    @if(Sentinel::getUser()->user_type == 'merchant' || (hasPermission('cash_on_delivery_charge')))
        <li class="nav-item" role="presentation">
            <a class="nav-link ps-0 {{strpos(url()->current(),'merchant/cod-charge') || strpos(url()->current(),'staff/cod-charge') ? 'active':''}}"
    ...
    @if(Sentinel::getUser()->user_type == 'merchant' || (hasPermission('manage_shops')))
        <li class="nav-item" role="presentation">
            <a class="nav-link ps-0 {{strpos(url()->current(),'merchant/shops') || strpos(url()->current(),'staff/shops') ? 'active':''}}"
    ...
    @if(Sentinel::getUser()->user_type == 'merchant' && @settingHelper('preferences')->where('title','read_merchant_api')->first()->merchant)
        <li class="nav-item" role="presentation">
            <a class="nav-link ps-0 {{strpos(url()->current(),'merchant/api-credentials') ? 'active':''}}"
    ...
    @endif
    <li class="nav-item" role="presentation">
        <a class="nav-link" href="javascript:void(0);" onclick="logout_user_devices('/logout-other-devices', '')" id="delete-btn">
