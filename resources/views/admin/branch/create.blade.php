        ...
        <div class="row justify-content-md-center">
            <div class="col-12 col-lg-6 col-md-8">
                <div class="header-top d-flex justify-content-between align-items-center">
                        <h3 class="section-title">{{ __('add') }} {{ __('branch') }}</h3>
                    <div class="oftions-content-right mb-12">
                        <div class="oftions-content-right">
                            <a href="{{ url()->previous() }}"
                                class="d-flex align-items-center btn sg-btn-primary gap-2"><i
                                    class="icon las la-arrow-left"></i><span>{{ __('back') }}</span>
                            </a>
                        </div>
                    </div>
                </div>
                <form action="{{ route('admin.branch.store') }}" class="form-validate" method="POST"
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="card-inner">
                                    <div class="mb-3">
                                        <label class="form-label" for="incharge">{{ __('manager') }} <span
                                                class="text-danger">*</span></label>
                                            <select class="without_search form-select form-control @error('manager') is-invalid @enderror" id="incharge"
                                                name="manager">
                                                <option value="">{{ __('select_manager') }}</option>
                                                @foreach ($users as $user)
                                                    <option value="{{ $user->id }}"
                                                        {{ old('incharge') ? ($user->id == old('incharge') ? 'selected' : '') : '' }}>
                                    ...
                                    <div class="mb-3">
                                        <label class="form-label" for="address">{{ __('address') }} <span
                                                class="text-danger">*</span></label>
                                            <textarea class="form-control @error('address') is-invalid @enderror" id="address" placeholder="{{ __('address') }}" name="address">{{ old('details') }}</textarea>
