        ...
        <div class="row justify-content-md-center">
            <div class="col-12 col-lg-6 col-md-8">
                <div class="header-top d-flex justify-content-between align-items-center mb-12">
                    <h3 class="section-title">{{ __('edit') }} {{ __('fund_transfers') }}</h3>
                ...
                <form action="{{ route('admin.fund-transfer.update') }}" method="POST"
                    enctype="multipart/form-data">
                    <input type="hidden" value="{{ $fund_transfer->id }}" name="id" id="transfer_id">
                    @csrf
                                ...
                                <div class="card-inner">
                                    <div class="mb-3">
                                        ...
                                        >
                                            ...
                                            @foreach ($bank_accounts as $account)
                                                <option value="{{ $account->id }}"
                                                    {{ $account->id == $fund_transfer->from_account_id ? 'selected' : '' }}>
                                                    ({{ __($account->method) }})
                                        ...
                                        <div class="text-success">
                                            <p>{{ __('current_payable_balance') }}: <span
                                                    id="current-balance">{{ setting('default_currency') }}{{ $balance }}</span>
                                            </p>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="without_search  form-control @error('to_account') is-invalid @enderror" name="to_account">
                                            ...
                                            @foreach ($bank_accounts as $account)
                                                <option value="{{ $account->id }}"
                                                    {{ $account->id == $fund_transfer->to_account_id ? 'selected' : '' }}>
                                                    ({{ __($account->method) }})
                                                    @if ($account->method == 'bank')
                                                        {{ $account->account_holder_name . ', ' . $account->account_no . ', ' . __($account->bank_name) . ',' . $account->bank_branch }}.
                                                    @elseif($account->method == 'cash')
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <div class="form-control-wrap focused">
                                            <input type="text" class="form-control date-picker @error('date') is-invalid @enderror" name="date"
                                                autocomplete="off"
                                                value="{{ isset($fund_transfer->date) ? date('Y-m-d', strtotime($fund_transfer->date)) : date('Y-m-d') }}">
                                        </div>
                                        @if ($errors->has('date'))
                                            <div class="invalid-feedback help-block">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="number" class="form-control @error('amount') is-invalid @enderror" id="fv-full-name"
                                            value="{{ old('amount') != '' ? old('amount') : $fund_transfer->amount }}"
                                            name="amount">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <textarea class="form-control" id="note" placeholder="{{ __('details') }}" name="note">{{ old('note') != '' ? old('note') : $fund_transfer->note }}</textarea>
                                    </div>
                                    <div class="row">
