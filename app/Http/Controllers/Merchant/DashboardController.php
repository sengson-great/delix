...
class DashboardController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        //  dd($total_parcel_count);
        $total_shop = Shop::where('merchant_id', \Sentinel::getUser()->merchant->id)->count();
        $total_staff = User::where('merchant_id', \Sentinel::getUser()->merchant->id)->where('user_type', 'merchant_staff')->count();
        ...
        $return_parcel = $this->returnParcel($start_date, $end_date)->withSum('account', 'amount')
            ->get();
        ...
        if ($total_parcel_count > 0) {
            ...
            $return_ratio = round(($return_parcel->count() * 100 / $total_parcel_count), 2);
        } else {
            $delivery_ratio = 0;
            $return_ratio = 0;
        }
        ...
        $data = [
            ...
            'returned_total_amount' => $return_parcel->sum('account_sum_amount'),
        ];
    ...
    {
        $data = Parcel::where('merchant_id', \Sentinel::getUser()->merchant->id)->whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date);
    ...
    public function processedParcel($start_date, $end_date)
    {
        $data = Parcel::whereDate('created_at', '>=', $start_date)
            ...
            ->where('merchant_id', \Sentinel::getUser()->merchant->id)
