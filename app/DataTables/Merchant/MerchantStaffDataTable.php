        ...
        public function dataTable(QueryBuilder $query): EloquentDataTable
        {
            return (new EloquentDataTable($query))
                ...
                ->addColumn('options', function ($query) {
                    return view('merchant.staffs.column.action', compact('query'));
                })->addColumn('user', function ($query) {
                    return view('merchant.staffs.column.user', compact('query'));
                })->addColumn('current_balance', function ($query) {
                    return view('merchant.staffs.column.current_balance', compact('query'));
                ...
                ->addColumn('status', function ($query) {
                    return view('merchant.staffs.column.status', compact('query'));
                })
                ->setRowId('id');
        }

        public function query(User $model, $pn = ''): QueryBuilder
        {
            $merchant_id = Sentinel::getUser()->merchant->id;
            $query = $model->where('user_type', 'merchant_staff')->where('merchant_id', $merchant_id)
